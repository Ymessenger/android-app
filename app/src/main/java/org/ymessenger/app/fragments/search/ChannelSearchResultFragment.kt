...
import org.ymessenger.app.adapters.ChannelsPagedAdapter
import org.ymessenger.app.data.remote.NetworkState
import org.ymessenger.app.viewmodels.GlobalSearchViewModel

class ChannelSearchResultFragment : BaseSearchTabFragment() {
    ...
    override fun onCreateView(
        ...
        val channelsPagedAdapter = ChannelsPagedAdapter(Glide.with(this)) {
            if (!canClick()) return@ChannelsPagedAdapter
            viewModel.openChannel(it.id)
        }
        view.recyclerView.adapter = channelsPagedAdapter

        view.swipeRefreshLayout.setOnRefreshListener {
            viewModel.refreshChannels()
        }

        subscribeUi(viewModel, channelsPagedAdapter)
        return view
    }

    private fun subscribeUi(
        viewModel: GlobalSearchViewModel,
        channelsPagedAdapter: ChannelsPagedAdapter
    ) {
        viewModel.channels.observe(this, Observer {
            channelsPagedAdapter.submitList(it)
        })

        viewModel.channelsRefreshState.observe(this, Observer {
